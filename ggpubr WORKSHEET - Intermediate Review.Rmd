...
```{r}
ggboxplot(na.omit(penguins), x = "species", y = ?,  # define data you want to plot
         add = ?, add.params = list(fill = ?)) + # we want a boxplot, add.params adds aesthetics to the ggpubr plot, in this case we want to change the fill color
  stat_compare_means(comparisons = ?, aes(label = "p.signif")) + # stat_compare_means adds p-values of your comparisons, aes(label = ) adds a label
  ...
  labs(x = ?, 
       y = ?) + # change axes labels; note that this is identical to ggplot
  theme(axis.title = element_text(face = ?)) # change axis titles face; again note that this is identical to ggplot
```
