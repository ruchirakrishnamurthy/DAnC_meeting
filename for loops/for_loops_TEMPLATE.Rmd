...
# create an object y to hold the answer by repeating the number 0 into a vector of the same length as x (i.e., 9 slots)

# for every number in the vector x, starting at the first variable in position 1 to the last variable in position 10
  
# add seven to it and store it in the vector y, our new values will overwrite y 
...
for() { # for element, i, from position 1 to the length of our holding vector
  
  ) # sum the i-th column in the dune dataset and place that answer in the holding vector in position i
...
for(i in 1:dim(mymat)[1]) {
  ...
  for(j in 1:dim(mymat)[2]) {
    ...
    mymat[fill_in_here] = fill_in_here
